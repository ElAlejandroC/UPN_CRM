...
async function cargarDatos() {
    try {
        const response = await fetch('http://192.168.100.253:3000/api/inscripciones');
...
async function confirmarEliminacion() {
    ...
    try {
        const response = await fetch('http://192.168.100.253:3000/api/borrar', {
            method: 'DELETE',
...
document.getElementById('agregar-form').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch('http://192.168.100.253:3000/api/agregar', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function abrirModalModificar(id) { // Recibe el ID
    ...
    try {
        ...
        const response = await fetch(`http://192.168.100.253:3000/api/obtenerFila/${idModificar}`);
...
async function modificarRegistro(idModificar) {
    ...
    try {
        ...
        const response = await fetch(`http://192.168.100.253:3000/api/modificar/${idModificar}`, {
            method: 'PUT',
            headers: {
...
function descargarPDF(id) {
    window.location.href = `http://192.168.100.253:3000/api/generar-pdf/${id}`;
}
